...
$queryUser = "SELECT * FROM tabel_user JOIN tabel_non_guru ON tabel_non_guru.nip = tabel_user.username WHERE username='".$username."' AND password=md5('".$password."') AND level='non guru'";
// $queryUser = "SELECT * FROM tabel_user JOIN tabel_non_guru ON tabel_non_guru.nip = tabel_user.username WHERE username='".$username."' AND level='non guru'";
...
if (mysqli_num_rows($result)>0) {
	...
	$data=array("username" => "", "password" => "", "id_non_guru" => "");
	echo json_encode($data);
}
