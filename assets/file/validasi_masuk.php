...
if($valid>0){
		if($valid == $valid2){
		    while ($periksa = mysqli_fetch_array($cek,MYSQLI_ASSOC)) {
		  		...
		  		$eksekusi = mysqli_query($con,"UPDATE tabel_absen SET sms_masuk = '-',valid_masuk = 'valid' 
		  			WHERE id_penempatan='$id_s' AND tgl='".$tgl2."'");
		  	}
		  	//echo "sip".$valid2.",".$valid;
		  	echo "sip";
		}else{
			//echo "error ".$valid2.",".$valid;
			echo "error";
		}
}else{
			//echo "error ".$valid2.",".$valid;
